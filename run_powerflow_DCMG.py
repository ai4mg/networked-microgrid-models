...
def compile_circuit() -> None:
    ...
    content = DSS_FILE.read_text(encoding="utf-8")

    fixed_lines = []
    for line in content.splitlines():
        is_relay_without_target = (
            line.strip().lower().startswith("new relay.")
            and "switchedobj" not in line.lower()
        )
        if is_relay_without_target:
...
def add_loads(load_specs: list[tuple[int, float, float]]) -> None:
...
def add_generators(gen_specs: list[tuple[int, float, float]]) -> None:
...
def add_storage(storage_specs: list[tuple[int, float, float, float]]) -> None:
...
def set_source_voltage(source_name: str = "source", pu: float = 1.0) -> None:
...
def run_powerflow(hour: int | None = None) -> bool:
    """Execute the power flow solution."""
    dss.Solution.Solve()

    hour_label = f"Hour {hour:02d}: " if hour is not None else ""
    if dss.Solution.Converged():
        print(f"[OK] {hour_label}power flow converged successfully")
        return True

    print(f"[ERROR] {hour_label}power flow did NOT converge")
    return False


def extract_bus_results() -> pd.DataFrame:
...
def extract_line_losses() -> pd.DataFrame:
...
def extract_load_results() -> pd.DataFrame:
...
ADDED_LOAD_44_SCALE_24H = [
    0.58, 0.57, 0.56, 0.56, 0.56, 0.54,
    0.53, 0.53, 0.54, 0.63, 0.76, 0.56,
    0.52, 0.52, 0.62, 0.59, 0.65, 0.58,
    0.61, 0.57, 0.54, 0.52, 0.55, 0.58,
]

ADDED_LOAD_108_SCALE_24H = [
    0.54, 0.57, 0.56, 0.55, 0.59, 0.59,
    0.56, 0.55, 0.54, 0.65, 0.83, 0.59,
    0.54, 0.55, 0.69, 0.68, 0.77, 0.66,
    0.66, 0.61, 0.54, 0.56, 0.60, 0.65,
]

ADDED_LOAD_SCALE_24H_BY_LOAD = {
    "add44": ADDED_LOAD_44_SCALE_24H,
    "add108": ADDED_LOAD_108_SCALE_24H,
}
...
def validate_24h_profile(name: str, profile: list[float]) -> None:
    """Confirm that each hourly profile has one scale for each hour."""
    if len(profile) != HOURS_PER_DAY:
        raise ValueError(
            f"{name} must contain exactly {HOURS_PER_DAY} values, got {len(profile)}"
        )


def validate_added_load_profiles(load_specs: list[tuple[int, float, float]]) -> None:
    """Confirm each added load has its own hourly scaling profile."""
    expected_load_names = set(load_names_from_specs(load_specs))
    configured_load_names = set(ADDED_LOAD_SCALE_24H_BY_LOAD)
    missing_load_names = expected_load_names - configured_load_names
    extra_load_names = configured_load_names - expected_load_names

    if missing_load_names:
        raise ValueError(
            "Missing added-load scaling profiles for: "
            + ", ".join(sorted(missing_load_names))
        )
    if extra_load_names:
        raise ValueError(
            "Configured added-load scaling profiles do not match added loads: "
            + ", ".join(sorted(extra_load_names))
        )

    for load_name, profile in ADDED_LOAD_SCALE_24H_BY_LOAD.items():
        validate_24h_profile(f"ADDED_LOAD_SCALE_24H_BY_LOAD[{load_name}]", profile)


def added_load_scale_row(hour: int) -> dict[str, float]:
    """Return summary columns for the added-load scale values at one hour."""
    return {
        f"{load_name}_Scale": profile[hour]
        for load_name, profile in ADDED_LOAD_SCALE_24H_BY_LOAD.items()
    }
...
def configure_storage_ratings(storage_ratings: dict[str, tuple[float, float]]) -> None:
...
def edit_loads(ratings: dict[str, tuple[float, float]], scale: float) -> None:
...
def edit_loads_by_name(
    ratings: dict[str, tuple[float, float]],
    scales_by_load: dict[str, float],
) -> None:
    for name, (base_kw, base_kvar) in ratings.items():
        scale = scales_by_load[name]
        dss.Text.Command(
            f"Edit Load.{name} kW={base_kw * scale:.6f} kvar={base_kvar * scale:.6f}"
        )


def edit_generators(ratings: dict[str, tuple[float, float]], scale: float) -> None:
    for name, (base_kw, base_kvar) in ratings.items():
        dss.Text.Command(
            f"Edit Generator.{name} kW={base_kw * scale:.6f} "
            f"kvar={base_kvar * scale:.6f} Enabled=Yes"
        )


def edit_storage(ratings: dict[str, tuple[float, float]], scale: float) -> None:
    if scale > 0:
        state = "DISCHARGING"
    ...
    for name, (rated_kw, _rated_kwh) in ratings.items():
        dss.Text.Command(
            f"Edit Storage.{name} kWrated={rated_kw:.6f} "
            f"{dispatch_setting} State={state} Enabled=Yes"
        )
...
def save_voltage_profile_plot(bus_df: pd.DataFrame, plot_file: Path) -> None:
    ...
    for hour in range(HOURS_PER_DAY):
        ...
        ax.axhline(
            HIGH_VOLTAGE_LIMIT_PU,
            color="#c44e52",
            linestyle="--",
            linewidth=0.8,
        )
        ax.axhline(
            LOW_VOLTAGE_LIMIT_PU,
            color="#c44e52",
            linestyle="--",
            linewidth=0.8,
        )
...
def concat_or_empty(frames: list[pd.DataFrame]) -> pd.DataFrame:
    """Concatenate frames when data exists; otherwise return an empty frame."""
...
def main() -> None:
    try:
        ...
        validate_added_load_profiles(ADDED_LOAD_SPECS)
        ...
        for hour in range(HOURS_PER_DAY):
            ...
            if not run_powerflow(hour):
                summary_rows.append(
                    {
                        "Hour": hour,
                        "Converged": False,
                        "Base_Load_Scale": BASE_LOAD_SCALE_24H[hour],
                        **added_load_scale_row(hour),
        ...
        bus_all = concat_or_empty(bus_frames)
        line_all = concat_or_empty(line_frames)
        load_all = concat_or_empty(load_frames)
        generator_all = concat_or_empty(generator_frames)
        storage_all = concat_or_empty(storage_frames)
